"""Logical 6x20 replay extension: three frozen baselines, v1 10000 / v2 20000 updates. One entry: audit, preflight, train, report, status.
...
PROJECT = SOURCE_ROOT.parents[2]
VERSION = SOURCE_ROOT.parent.name.rsplit('_', 1)[1]
OLD_PER = 48 if VERSION == 'v1' else 56
NEW_PER = 64 - OLD_PER
MAX_UPDATES = 10000 if VERSION == 'v1' else 20000
ARCHITECTURES = ('small_cnn_v2', 'mini_inception_v2', 'resnet20_v2')
BASELINES = {
...
def settings(old_config):
    return {
        "experiment": f"cnn_replay_update_{VERSION}/logical6x20/{ARCH}", "seed": SEED, "groups": ["G-S", "F-S"],
...
def preflight():
    ...
    max_diff = 0.0
    other = SOURCE_ROOT.parent.parent / "cnn_replay_update_v1/logical6x20" / ARCH / "baseline/metrics.json"
    if VERSION == "v2" and other.exists():
        require(read(other) == baseline, "v1/v2 own frozen baseline predictions differ")
    save(HERE / "audit/preflight.json", {"status": "passed", "input_shape": [64,2,6,20], "replay": [OLD_PER,NEW_PER],
...
def report():
    from report_results import run
    run(SOURCE_ROOT)
...
def suite(resume=False):
    ...
    for version in ("v1", "v2"):
        folder = PROJECT / "experiments" / f"cnn_replay_update_{version}" / "logical6x20"
        for architecture in ARCHITECTURES:
            model_root = folder / architecture
            for command in ("preflight", "train"):
                if command == "preflight" and (model_root / "audit/preflight.json").exists():
                    continue
                args = [sys.executable, "-u", str(folder / "update.py"), command, "--architecture", architecture]
                if command == "train" and resume:
                    args.append("--resume")
                save(SOURCE_ROOT / "suite_progress.json", {"status":"running", "version":version,
                     "architecture":architecture, "command":command, "logs":str(logs)})
                print("SUITE", version, architecture, command, flush=True)
                log_path = logs / f"{version}_{architecture}_{command}.log"
                with log_path.open("ab") as stream:
                    completed = subprocess.run(args, stdout=stream, stderr=subprocess.STDOUT)
                if completed.returncode:
                    save(SOURCE_ROOT / "suite_progress.json", {"status":"failed", "version":version,
                         "architecture":architecture, "command":command, "exit_code":completed.returncode,
                         "log":str(log_path)})
                    raise RuntimeError(f"Suite stopped; inspect {log_path}")
    save(SOURCE_ROOT / "suite_progress.json", {"status":"complete", "runs":12, "test_used":False})
    print("SUITE complete: 12 runs", flush=True)


def main():
    global ARCH, HERE, MODEL, CHECKPOINT_SHA
    for stream in (sys.stdout, sys.stderr):
        if hasattr(stream, "reconfigure"):
            stream.reconfigure(encoding="utf-8")
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("command", choices=("audit", "preflight", "train", "report", "status", "suite"), nargs="?", default="status")
    parser.add_argument("--architecture", choices=ARCHITECTURES, default="small_cnn_v2")
    ...
    if args.command == "suite":
        ...
        for group in ("G-S", "F-S"):
            ...
            if path.exists():
                p=read(path)
                print(ARCH, group, p["status"], p.get("step",p.get("updates")))
            else:
                print(ARCH, group, "not started")
