...
def rows(path):
    with relocated(path).open(encoding='utf-8-sig',newline='') as f:
        return list(csv.DictReader(f))


def table(path, values):
    with relocated(path).open('w',encoding='utf-8-sig',newline='') as f:
...
def run(root):
    root=Path(root);version=root.parent.name.rsplit('_',1)[1];out=report_dir(root);out.mkdir(exist_ok=True)
    ...
    if version=='v2':
        comparison=[]
        for v in ('v1','v2'):
            location=root.parent.parent/f'cnn_replay_update_{v}'/'models'
            for arch in ARCHS:
                for group in ('G-S','F-S'):
...
def verify_saved_models(root):
    ...
    for arch in ARCHS:
        ...
        assert read(audit_dir(u.HERE) / 'preflight.json')['checkpoint_next_update_bit_exact']
