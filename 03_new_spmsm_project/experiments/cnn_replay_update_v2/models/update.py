...
def preflight():
    ...
    max_diff = 0.0
    other = SOURCE_ROOT.parent.parent / "cnn_replay_update_v1/models" / ARCH / "baseline/metrics.json"
...
def suite(resume=False):
    ...
    for version in ("v1", "v2"):
        folder = PROJECT / "experiments" / f"cnn_replay_update_{version}" / "models"
        for architecture in ARCHITECTURES:
            model_root = folder / architecture
            for command in ("preflight", "train"):
                if command == "preflight" and (audit_dir(model_root) / 'preflight.json').exists():
                    continue
