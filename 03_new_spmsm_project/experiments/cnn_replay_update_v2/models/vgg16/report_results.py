...
V1 = _LAYOUT_PROJECT / "experiments/cnn_replay_update_v1/models/vgg16"
OUT = report_dir(ROOT)
...
def read(path):
    return json.loads(relocated(path).read_text(encoding="utf8"))
...
def table(path, rows):
    with relocated(path).open("w", encoding="utf-8-sig", newline="") as stream:
...
def verify_outputs():
    ...
    restoration = read(audit_dir(ROOT) / 'v1_report_restoration.json')
    for name, expected in read(audit_dir(ROOT) / 'v1_reference.json')['sha256'].items():
        if name == restoration['file']:
            assert expected == restoration['original_sha256']
            assert sha(audit_dir(ROOT) / 'v1_interpretation_before_restore.md') == expected
            expected = restoration['restored_sha256']
        assert sha(_LAYOUT_PROJECT / 'experiments/cnn_replay_update_v1' / name) == expected, f'Unexpected v1 change: {name}'
    assert sha(ROOT / 'update.py') == sha(ROOT / 'source_snapshot/update.py')
    assert sha(_LAYOUT_PROJECT / cfg['initial_checkpoint']) == cfg['initial_checkpoint_sha256']
    ...
    save(audit_dir(ROOT) / 'final_output_verification.json',{'passed':True,'v1_configs_results_checkpoints_unchanged':True,
