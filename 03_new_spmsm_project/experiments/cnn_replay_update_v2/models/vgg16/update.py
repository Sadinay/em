"""G-S/F-S v2 replay update: 87.5% old, fixed 5000 successful updates. One entry: audit, preflight, train, report, status.
...
def settings(old_config):
    return {
        "experiment": "cnn_replay_update_v2", "seed": SEED, "groups": ["G-S", "F-S"],
        ...
        "physical_batch_size": old_config["physical_batch_size"], "old_per_update": 56, "new_per_update": 8,
        "max_updates": 5000, "validate_every": 500, "checkpoint_every": 100,
        "fixed_budget": True, "gradient_accumulation_steps": 8, "improvement_epsilon": 1e-12,
        "old_mae_limit_multiplier": 1.05,
        "selection": "both old MAEs <= 1.05*f0; then minimize new-dev dual-target mean standardized MSE",
        "early_stop": "disabled; complete 5000 successful optimizer updates unless an abnormal failure occurs",
        ...
        "settings_source": "v1 settings retained except 56:8 replay and fixed 5000 updates; user-prespecified development experiment",
    ...
    def __iter__(self):
        for step in range(self.start, self.stop):
            for micro in range(8):
                yield np.concatenate((self.old_plan[step, micro*7:(micro+1)*7],
                                      self.new_plan[step, micro:micro+1] + self.old_count)).tolist()
...
def perform_update(model, renderer, spec, mean, std, device, optimizer, scaler, batches):
    ...
    while True:
        ...
        for _, bits, target in batches:
            ...
            sums += [float(loss_sum.detach()), float(errors[:7].sum().detach()), float(errors[7:].sum().detach())]
        ...
        if scaler.get_scale() >= previous_scale:
            ...
            return {"mse": sums[0]/128, "old_mse": sums[1]/112, "new_mse": sums[2]/16,
...
def preflight():
    ...
    batches = list(loader)
    require(len(batches) == 16 and all(int((b[0] < 40000).sum()) == 7 for b in batches), "Replay microbatch ratio failed")
    ...
    save(audit_dir(HERE) / 'preflight.json', {"status": "passed", "input_shape": [8,8,224,224], "replay": [56,8],
...
def train_group(group, cfg, data, meta, resume):
    ...
    started = time.perf_counter()
    stop_reason = "fixed_5000_successful_updates"
    try:
