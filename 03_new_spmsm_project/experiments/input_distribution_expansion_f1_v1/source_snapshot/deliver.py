...
def registry():
    ...
    for root in [PROJECT/'experiments/cnn_replay_update_v2',*[PROJECT/'experiments/cnn_replay_update_v2/logical6x20'/n for n in ('small_cnn_v2','resnet20_v2')]]:
    ...
    for rp in sorted((PROJECT/'experiments/cnn_shrink_perturb_v1').glob('alpha_*/runs/*/result.json')):
        for kind in ('best_unconstrained','best_feasible'):
            cp=rp.parent/(kind+'.pt')
