...
BETA = 0.01
SP_SEED = 20261914
CONTROL = PROJECT / 'experiments/cnn_replay_update_v2/logical6x20/small_cnn_v2'
...
def sp_preflight(cfg, data, meta):
    ...
    for name in ('old_replay_indices.npy','new_replay_indices.npy'):
        require(sha(HERE/'audit'/name) == sha(CONTROL/'audit'/name), 'Control sampling plan differs')
    runtime=read(CONTROL/'audit/runtime.json')
    ...
    path=HERE/'audit/preflight/sp_roundtrip.pt'
    ...
    path.unlink()
    save(HERE/'audit/sp_preflight.json',{'passed':True,'identity_parameters_and_next_update_exact':True,
...
def suite(resume=False):
    ...
    for alpha in (.95,.80,.50):
        ...
        for command in ('preflight','train'):
            if command=='preflight' and (SOURCE_ROOT/label/'audit/sp_preflight.json').exists():
                require(read(SOURCE_ROOT/label/'audit/sp_preflight.json')['source_sha256']==sha(__file__),'SP preflight source changed')
                continue
...
def main():
    ...
    if args.command=='suite':suite(args.resume)
        ...
        require(read(HERE/'audit/sp_preflight.json')['passed'],'Run SP preflight first')
        train(args.resume)
