...
[查看MSE图与结论](../../reports/small_cnn_v2/MSE对比说明.md)。绘图源码 `plot_mse_comparison.py`；读取已保存验证指标，只生成PNG，不训练、不接触测试集。

## 事后容限放宽复核

[6%容限分析](../../reports/small_cnn_v2/6pct容限复核.md)：用户要求小幅放宽后，将5%/6%/7.5%统一应用于已有G/F及无SP验证轨迹。6%下α=.80 F-S的9500步合格；原5%结果保留。源码 `review_tolerance.py`，不重训，不修改模型/数据或原accepted字段。
