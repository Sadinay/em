"""Build an SPMSM FEM model from one audited 120-bit chromosome."""
...
DEFAULT_TEMPLATE = ROOT / "femm_zone" / "models" / "SPMSM_discrete.fem"
DEFAULT_OUTPUT = ROOT / "femm_zone" / "results" / "generated_examples" / "history_gene"
GENE_COUNT = 120
COPIES_PER_GENE = 4
ANGULAR_CELLS = 20
RADIAL_CELLS = 6
...
def history_gene(mat_path: Path, state_index: int, population_row: int, source: str = "population_all") -> tuple[np.ndarray, dict[str, Any]]:
    ...
    provenance = {
        ...
        "stored_results": {
            ...
            "delta_t_nm": float(np.asarray(data["DeltaT_all"])[population_row, state_index]),
...
def main() -> None:
    ...
    parser.add_argument("--state-index", type=int, default=200)
    parser.add_argument("--population-row", type=int, default=0)
    parser.add_argument("--source", choices=("population_all", "population_noChange_all"), default="population_all")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    args = parser.parse_args()
    bits, provenance = history_gene(args.mat, args.state_index, args.population_row, args.source)
    model = build_topology(bits, args.template, args.mat, args.output_dir, provenance)
    print(f"Generated: {model}")
    print(f"Gene hash: {genotype_sha256(bits)}")
    print(f"PM cells: {int(bits.sum())}; Air cells: {int((bits == 0).sum())}")
