# 6个SPMSM基因的可调FEMM模型（Min Angle=25°）

这里包含6个彼此不同的120位基因，每个基因对应一个独立 `.fem` 文件。文件名中的 `g` 是 MAT 历史代数，`row` 是 `population_all` 的种群行号，均为0-based。本目录是重新求解的 `Min Angle=25°` 版本；原来的 `Min Angle=15°` 版本保留在平行目录中。
