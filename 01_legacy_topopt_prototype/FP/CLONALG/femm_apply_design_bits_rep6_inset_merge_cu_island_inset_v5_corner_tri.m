function femm_apply_design_bits_rep6_inset_merge_cu_island_inset_v5_corner_tri( ...
...
% 铜并区（用于"相邻铜格合并成整块"：删内部边 + 合并标签/匝数）
key_cu = nan(Nd,1);
key_cu(mat_code == 2) = 300;
keep_cu_label = keep_one_label_per_region(nr, nt, key_cu, ir_of_k, it_of_k);
cu_comp_size = component_size_per_cell(nr, nt, key_cu, ir_of_k, it_of_k);

% 空气带并区（把相邻空气带合并，并合并空气材料标签）
...
for s = 1:nSector
    ...
    for k = 1:Nd
        ...
        if code == 2
            if has_air_band_cell(k) && keep_air_band_label(k)
        ...
        % ===== 铜几何：先逐格生成，再统一删铜并区内部边界 =====
        add_copper_cell_boundary_island_inset(k, s, isMirror, Cs, c0, ...
            nr, nt, r_edges, theta_edges, alpha, alpha, groupIdCuGeom, ...
            ir_of_k, it_of_k, is_copper_grid);
    end

    % ===== 铜并区：删内部边界/孤立点，把相邻铜格合并为整块 =====
    delete_internal_boundaries_sector(r_edges, theta_edges, c0, Cs, isMirror, nr, nt, key_cu, ir_of_k, it_of_k);
    remove_isolated_nodes_sector(r_edges, theta_edges, c0, Cs, isMirror, nr, nt, key_cu, ir_of_k, it_of_k);

    % ===== 铜 label：每个铜连通块只保留一个标签，匝数按块内格子数合并 =====
    for k = 1:Nd
        if mat_code(k) ~= 2 || ~keep_cu_label(k)
            continue;
        end
        phaseId = pid(k);
        if phaseId == 0
            ...
            turns = cu_comp_size(k) * turns_per_circ(phaseId);
        end
...
if need_ul || need_ur || need_dl || need_dr
    ...
    if need_dl
        pr = [pr, ri2 - dr, ri2 - dr, ri2];
        pt = [pt, ti1,      ti1 + dth, ti1 + dth];
    else
        pr = [pr, ri2];
        pt = [pt, ti1];
    end
    ...
    if need_dr
        pr = [pr, ri2,      ri2 - dr, ri2 - dr];
        pt = [pt, ti2 - dth, ti2 - dth, ti2];
    else
        pr = [pr, ri2];
        pt = [pt, ti2];
    end
    ...
    if need_ur
        pr = [pr, ri1 + dr, ri1 + dr, ri1];
        pt = [pt, ti2,      ti2 - dth, ti2 - dth];
    else
        pr = [pr, ri1];
        pt = [pt, ti2];
    end
